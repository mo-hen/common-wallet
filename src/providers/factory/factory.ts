import { Action, txType, txToType, unitType, unitMessagePayloadType } from "./../types/types";
...
export class FactoryProvider {
    ...
    constructor(private logger: Logger, private profile: ProfileProvider) {
        this.data = <txType>{};
    }
    ...
    unpackUnit(unit: unitType): txType {
        ...
        if (unit.authors[0].address == this.profile.profileTotal.wallet.address) {
            // 支付行为
            ...
            unit.messages.forEach(item => {
                if (item.app == "text") {
                    ...
                    (<unitMessagePayloadType>item.payload).outputs.forEach(output => {
                        if (output.address == this.profile.profileTotal.wallet.address) {
                            let outputItem: txToType = {
                                assetId: (<unitMessagePayloadType>item.payload).asset || null,
                                address: output.address,
                                amount: output.amount
