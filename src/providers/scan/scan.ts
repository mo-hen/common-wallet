...
export class ScanProvider {
    public text: string; // 二维码内容
    public scannerVisible: boolean; // 扫码是否可用
    public lightEnabled: boolean; // 是否开启闪光灯
    public frontCameraEnabled: boolean; // 是否开启前置摄像头
    public isDesktop = !this.platform.isCordova; // 是否是桌面系统
    public isAvailable: boolean = true; // 服务是否可用
    public hasPermission: boolean = false; // 是否有权限
    public isDenied: boolean = false; // 是否被拒绝
    public isRestricted: boolean = false; // 是否受限
    public canEnableLight: boolean = false; // 可以打开闪光灯
    public canChangeCamera: boolean = false; // 可以改变摄像头
    public canOpenSettings: boolean = false; // 可以打开设置
    public backCamera: boolean = true; // 默认后置摄像头
    public initializeStarted: boolean = false; // 初始化是否已开始
    public initializeCompleted: boolean = false; // 初始化是否已完成
    ...
    private checkCapabilities(status) {
        this.logger.debug(
            "scannerService is reviewing platform capabilities..."
        );
        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.lightEnabled) {
                this.qrScanner
                    ...
                    .catch(err => {
                        this.logger.error(
                            "Scan Provider Error (disableLight)",
                            err
                        );
                        return reject(err);
                    });
                ...
                this.qrScanner
                    ...
                    .catch(err => {
                        this.logger.error(
                            "Scan Provider Error (enableLight)",
                            err
                        );
                        return reject(err);
                    });
        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.frontCameraEnabled) {
                this.qrScanner
                    ...
                    .catch(err => {
                        this.logger.error(
                            "Scan Provider Error (useBackCamera)",
                            err
                        );
                        return reject(err);
                    });
                ...
                this.qrScanner
                    ...
                    .catch(err => {
                        this.logger.error(
                            "Scan Provider Error (useFrontCamera)",
                            err
                        );
                        return reject(err);
                    });
